...
{
    public class QueryBuilderr : IDisposable
    {
        SqliteConnection? connection;

        public QueryBuilderr(string dbPath)
        {
        ...
        // Delete operation to remove a record from a SQLite table
        ...
        // Create method of T that allows an object to be inserted
        ...
        public void Create<T>(T obj) where T : IClassModel
        {
            ...
            for (int i = 1; i < properties.Length; i++) // Skip the first property (Id)
            {
                property = properties[i];
                // Format DateTime for the DB (reverse the process in Read)
                // Quotation marks are necessary for the date format
                if (property.PropertyType == typeof(DateTime))
                {
                    values.Add($"\"{((DateTime)property.GetValue(obj)).Year}-{((DateTime)property.GetValue(obj)).Month}-{((DateTime)property.GetValue(obj)).Day}\"");
                }
                // Format string for the DB
                // Quotation marks are necessary for the text format
                else if (property.PropertyType == typeof(string))
                {
                    values.Add($"\"{property.GetValue(obj).ToString()}\"");
                }
                // Format other data types for the DB
                else
                {
                    values.Add(property.GetValue(obj).ToString());
                }
        ...
        // DeleteAll method of T that allows all records to be deleted from a table
